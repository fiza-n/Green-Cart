...
const getCart = () => {
    ...
    for(let item in cartItems){
        ...
        if(product){
            product = { ...product, quantity: cartItems[item] }
        ...
        <div className="flex flex-col md:flex-row mt-16">
            <div className='flex-1 max-w-4xl'>
                ...
                {cartArray.map((product, index) => (
                    <div key={index} className="grid grid-cols-[2fr_1fr_1fr] text-gray-500 items-center text-sm md:text-base font-medium pt-3">
                        <div className="flex items-center md:gap-6 gap-3">
                            <div onClick={() => {navigate(`/all-products/${product.category.toLowerCase()}/${product._id}`); scrollTo(0,0);}} className="cursor-pointer w-24 h-24 flex items-center justify-center border border-gray-300 rounded overflow-hidden">
                                <img className="max-w-full h-full object-cover" src={(product.images ?? product.image)?.[0]} alt={product.name} />
                            </div>
                            <div>
                ...
                <button className="group cursor-pointer flex items-center mt-8 gap-2 text-primary font-medium">
                   <img onClick={() => {navigate("/all-products"); scrollTo(0,0)}} src={assets.arrow_right_icon_colored} alt="arrow" className='group-hover:-translate-x-1 transition' />
                    Continue Shopping
                </button>
